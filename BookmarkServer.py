...
def CheckURI(uri, timeout=5):
    ...
    try:
        long_url = requests.get(uri, timeout=timeout)
        return long_url.status_code == 200
...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Cotent-type', 'text/plain; charset=utf-8')
            self.end_headers()
        ...
        if CheckURI(longuri):
            ...
            self.end_headers()
            self.wfile.write(
                "Could not fetch URI '{}'. Sorry!".format(longuri).encode(())
            )
